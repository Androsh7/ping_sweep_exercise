...
# Project libraries
from src.ping import ping_range, ping_worker, ping
...
async def test_ping_worker():
    ...
    task = asyncio.create_task(ping_worker(name="test", ip_queue=ip_queue, reachable_ips=reachable_ips, timeout_ms=1000, progress=progress_bar))
    await ip_queue.join()
    task.cancel()
    await task
...
async def test_ping_range():
    ip_address_range = (IPv4Address("127.0.0.1"), IPv4Address("127.0.0.2"), IPv4Address("127.0.0.3"), IPv4Address("127.0.0.4"), IPv4Address("127.0.0.5"), )
